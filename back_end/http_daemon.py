...
#implements a webserver that talks to the internet
...
class MyRequestHandler(BaseHTTPRequestHandler):
    ...
    # responds to a GET request: Open the file and send the file back
    def do_GET(self) -> None:
        # ip_address = self.client_address[0]

        # Parse url (take the url and breaks it into parts)
        url_parts = urlparse.urlparse(self.path)
        filename = url_parts.path
        if filename[0] == '/':
            filename = filename[1:]

        # Parse query ('?name=value&...' list of name value pairs in URL sent to the GET REQUEST)
    ...
    # Responds to POST requests: better way to send stuff (secret, infomration not in url)
